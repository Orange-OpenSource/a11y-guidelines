document.addEventListener("DOMContentLoaded", function (event) {
    ...
    doXHR('/assets/json/checklist/tests-web-' + lang + '.json', function (errFirst, responseFirst) {
        if (errFirst) {
            reqError();
        }
        return doXHR('/assets/json/checklist/tests-concepteur-' + lang + '.json', function (errSecond, responseSecond) {
            if (errSecond) {
                reqError();
            }
        ...
        let app = new function () {
            ...
            this.UpdateFeedback = function (activeFilter, nbTests, isRefresh) {
                ...
                if (activeFilter) {
                    ...
                    htmlFeedback = '<p class="mb-0"><b>' + nbTests + '</b> ' + test + ' ' + translate('withCurrentFilters') + '</p><button type="button" class="btn btn-secondary btn-sm mt-2 mb-3" id="reinitLink">' + translate('reinitFilters') + '</button>';
            ...
            this.FetchAll = function (currentRefTests) {
                ...
                const restoredUl = originalUl.cloneNode(true);
                toc.replaceChild(restoredUl, toc.querySelector('ul'));
                ul = toc.querySelector('ul');
                ul.querySelectorAll('li').forEach(li => {
                    if (!themeTab.includes(li.textContent.trim())) {
                        li.remove();
                    }
                });
