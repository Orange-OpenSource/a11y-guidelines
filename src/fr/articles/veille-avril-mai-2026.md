---
title: "Veille #A11y Avril-Mai 2026"
abstract: "Florilèges de ressources en #A11y, Avril-Mai  2026"
date: "2026-06-08"
tags:
  - veille
---

# Veille accessibilité numérique Avril - Mai 2026

La journée de la sensibilisation à l'accessibilité ou Global Accessibility Awareness Day (GAAD) fête ses  15 ans, 21 Mai 2026:
"Is the internet more accessible than it was ten years ago?" (en) : https://intopia.digital/global-accessibility-pulse-2026/

Les suites de la plainte pour non accessibilité contre des géants de la grande distribution, grosse désillusion : https://apidv.org/actualites/decision-du-tribunal-judiciaire-de-lille et https://droitpluriel.fr/decision-du-tribunal-judiciaire-de-lille-le-6-mai-2026apidv-et-droit-pluriel-c-auchan-e-commerce-une-interpretation-gravement-erronee-du-droit-de-laccessibilite-numerique/.

Le 31 mars, c'était la première édition d'A11y Ouest, nous étions là pour Orange, en attendant un retour en 2027 ade A11y Paris, un compte-rendu : https://access42.net/compte-rendu-A11y-ouest-2026/.

## L’accessibilité dans les lois, les normes et recommandations

- Prioriser les efforts pour l'ADA title II (en) : https://vispero.com/resources/ada-title-ii-accessibility-prioritization-guide/
- Point de vue des intéressés sur l'EAA : https://www.anysurfer.be/fr/blog/detail/european-accessibility-act-le-point-de-vue-des-personnes-en-situation-de-handicap
- Trucs cool dans le futur ARIA 1.3 (en) : https://afixt.com/cool-new-stuff-coming-in-aria-1-3/
- Bonnes intentions de l'ARCOM : https://www.arcom.fr/actualites/projet-strategique-2026-2028-de-larcom-une-regulation-du-numerique-plus-forte-proteger-les-francais
- Tout sur WCAG 3.1.2 Language of parts (en) : https://tarnoff.info/2026/05/11/A11y-101-3-1-2-language-of-parts/

## Stratégie de mise en accessibilité

- Stratégie de l'UE en matière d'accessibilité et d'inclusion (en) : https://commission.europa.eu/news-and-media/news/boosting-accessibility-and-inclusion-persons-disabilities-2026-05-06_en
- Créer des documents numériques accessibles, introduction (en) : https://tetralogical.com/blog/2026/05/13/creating-accessible-documents/
- Auditer en fin de cycle n'est pas la solution, shift left (en) : https://buttondown.com/access-ability/archive/you-can-t-audit-your-way-into-accessibility/
- Surcouches d'A11y, c'est mal, on en reparle (en) : https://the-word.news/2026/0402/accessibility-overlays-create-problematic-quick-fixes/
- Un an après l'EAA, réflexions (en) : https://gbbns.co/journal/accessibility-problem-isnt-design/
- Il faut des gens dédiés à l'A11y (en) : https://gbbns.co/journal/design-brief-that-never-mentioned-disabled-people/
- La dette A11y par rapport à celle de l'UX, le coût est bien supérieur, à lire (en) : https://www.accessux.com.au/blog/design-debt-vs-accessibility-debt-why-fixing-it-early-saves-millions/
- Pas de secret, l'A11y dès le départ, intéressant (en) : https://annaecook.com/writing/2026/ai-doesnt-fix-accessible-systems-it-depends-on-them

## Tester l’accessibilité

- Critère 2.5.8 Taille de la cible (en) : https://tarnoff.info/2026/04/06/A11y-101-2-5-8-target-size/
- Plus loin avec Playright que Axe ou Lighthouse (en) : https://247webdevs.blogspot.com/2026/05/why-keyboard-users-cant-scroll-your.html
- Wcag 3.0 et le calcul du rapport de contraste (en) : https://adrianroselli.com/2026/04/wcag3-contrast-as-of-april-2026.html
- Tester avec des ultilisateurs en situation de handicap (en) : https://A11ywithdiana.substack.com/p/how-to-organize-a-testing-with-users?publication_id=5254847&post_id=192762953&isFreemail=true&r=6k8koq&triedRedirect=true
- Le clavier et A11y (en) : https://www.dennisdeacon.com/accessibility/keyboard-accessibility-the-clearest-signal-of-digital-maturity/

## Points techniques

- Le sessionTimeOut et A11y, complet (en) : https://www.smashingmagazine.com/2026/04/session-timeouts-accessibility-barrier-authentication-design/
- Discussion sur l'attribut autofocus, à lire (en) : https://github.com/whatwg/html/issues/12436
- Au-delà de la conformité, évitons les éléments "br" : https://www.anysurfer.be/fr/blog/detail/balance-ton-br
- Rendre les containers avec overflow accessibles au scroll (en) : https://247webdevs.blogspot.com/2026/05/why-keyboard-users-cant-scroll-your.html
- aria-label, oui mais pas n'importe où (en) : https://matuzo.at/blog/2026/aria-label-generic-elements
- Cacher des éléments, les différentes méthodes et quand les utiliser (en) : https://vispero.com/resources/not-all-hidden-content-is-equal-how-different-hiding-methods-impact-accessibility/
- Emojis accessibles (en) : https://blog.pope.tech/2026/04/01/making-emojis-and-icons-screen-reader-accessible/
- Box-shadow à la place de outline, attention à l'A11y (en) : https://matuzo.at/blog/2026/box-shadow-no-alternative-to-outline
- Bien faire une structure de titres dans la page (en) : https://intopia.digital/articles/giving-pages-a-clear-shape-by-using-headings/

## Retour d'expérience

- Réflexions suite à 2026 WebAIM Million report (en) : https://webaim.org/blog/tolerating-inaccessibility/
- Passionante discussion autour du aria-modal et WCAG 1.3.1 info and relationships (en) : https://www.neilosman.com/posts/aria-modal-programmatically-determined/
- 9 poncifs sur l'A11y et leurs contre-arguments (en) : https://stephaniewalter.design/blog/9-accessibility-myths-and-pushbacks-and-how-to-answer-them/
- Bibliothèques et A11y : https://www.accessibilites.abf.asso.fr/2026/05/11/laccessibilite-un-defi-collectif-pour-les-bibliotheques/
- Pas de quoi se glorifier du GAAD (en) : https://buttondown.com/access-ability/archive/gaad-2026-not-much-to-celebrate-yet/
- GAAD, colère et A11y, si vrai (en) : https://buttondown.com/nic-steenhout/archive/new-podcast-gaad-and-purposeful-anger/
- Rappels sur l'utilisation d'ARIA, bien utile (en) : https://www.dennisdeacon.com/accessibility/aria-done-right-why-accessibility-starts-with-better-decisions-not-more-code/
- De l'IA pour créer des sites, on le savait, l'A11y n'y est pas (en) : https://axesslab.com/lovable/
- Retour d'un testeur en situation de handicap, à lire : :https://access42.net/references-clients/tests-utilisateurs-accessibilite-marjolaine-rayer/

## Appli mobile, web mobile

- Drag'n drop sur Android et A11y (en) : https://buttondown.com/inclusive-android-apps/archive/inclusive-android-apps-5-the-problem-of-drag-and-and-drop/
- Apple dévoile de nouvelles fonctionnalités d'accessibilité et des mises à jour optimisées par Apple Intelligence (en) : https://www.apple.com/newsroom/2026/05/apple-unveils-new-accessibility-features-and-updates-with-apple-intelligence/
- Samsung remplace sa version de TalkBack par celle de Google dans la bêta de One UI 9 (en) : https://accessibleandroid.com/samsung-replaces-its-talkback-version-with-googles-in-one-ui-9-beta/
- Rachele DiTullio nous gratifie de quatre très bons articles sur l'utilisation des aides techniques et fonctionnalités indispensables sur iPhone (en) : 
  - https://racheleditullio.com/blog/2026/05/at-for-iphone-full-keyboard-access/
  - https://racheleditullio.com/blog/2026/05/at-for-iphone-voice-control-speech-input/
  - https://racheleditullio.com/blog/2026/05/at-for-iphone-voiceover-screen-reader/
  - https://racheleditullio.com/blog/2026/05/4-ios-display-settings-to-check-your-app-with/
- Vérifier l'accessibilité avec SwiftUI Previews (en) : https://mobileA11y.com/blog/swiftui-preview-testing/

## Aides techniques

- Liste de quelques aides techniques (en) : https://vale.rocks/posts/digital-accessibility-technologies

## Ergonomie, UI & UX

- Est-ce nécessaire ? (en) : https://alan.com/en/blog/healthy-business/a/is-this-really-needed
- Designers une proposition pour faire plus A11y (en) : https://alistapart.com/article/good-designers-bad-websites-a-proposal/
- 10 principales erreurs d'A11y dans le webdesign (en) : https://www.levelaccess.com/blog/five-most-common-accessibility-errors-in-software-design-and-development/

## Intelligence Artificielle (IA)

- Un agent IA github pour l'A11y, retour d'expérimentation, pas mal (en) : https://github.blog/ai-and-ml/github-copilot/building-a-general-purpose-accessibility-agent-and-what-we-learned-in-the-process/
- AI et A11y, tour d'horizon, à lire (en) : https://annaecook.com/writing/2026/ai-doesnt-fix-accessible-systems-it-depends-on-them
- Encore un tour d'horizon mais plus complet en deux parties (en) : 
  - https://onsman.com/the-impact-of-ai-on-digital-accessibility-part-i/
  - https://onsman.com/the-impact-of-ai-on-digital-accessibility-part-ii/
- Un Agent Skill (en cours de création) pour les développeurs iOS, à tester (en): https://github.com/dadederk/iOS-Accessibility-Agent-Skill
- Ne comptez pas sur l'IA pour générer vos alternatives d'images (en) : https://silktide.com/blog/the-downsides-of-ai-alt-text/
- Chatbot AI spécifique A11y par WebAim (en) : https://webaim.org/AIMee/
- Les stéréotypes sont des biais cognitifs (en) : https://A11ywithdiana.substack.com/p/did-you-ever-think-about-how-the
- Tester l'accessibilité au clavier avec l'IA (en) : https://html5accessibility.com/stuff/2026/04/29/keyboard-testing-with-the-power-of-ai/
- Une évaluation des LLM et A11y (en) : https://microsoft.github.io/A11y-llm-eval-report/
- Modern Web Guidances de Google, à éviter (en) : https://adrianroselli.com/2026/05/maybe-dont-rely-on-googles-modern-web-guidance.html

## Ressources

- Des améliorations pour de la 3D su le web et A11y (en) : https://cerovac.com/A11y/2026/04/new-hope-for-better-accessibility-of-3d-experiences-on-the-web/
- Les formulaires A11y (en) : https://www.dennisdeacon.com/accessibility/accessible-forms-are-a-business-imperative-not-a-compliance-checkbox/
- Rendre A11y les graphes (en) : https://www.tempertemper.net/blog/do-graphs-and-charts-need-to-be-accessible 
- Des personas en situation de handicap (en) : https://knowaboutaccessibility.org/tag/personas/
- Concevoir pour les aveugles (en) : https://tetralogical.com/blog/2026/04/02/designing-for-people-who-are-blind/
- Les fondamentaux de l'A11y, la base (en) : https://inklusivo.nl/accessibility-fundamentals/
- Ressources A11y pour designers (en) : https://accessibility.tools/
- Deux guides sur l'accessibilitié des événements et de la communication : https://lesdevalideuses.org/ressources-2/guides-accessibilite-2026/
- Les dix erreurs classiques d'A11y en conception et dev (en) : https://www.levelaccess.com/blog/five-most-common-accessibility-errors-in-software-design-and-development/
- Handicap cognitif et A11y, bien complet (en) : https://mn.gov/mnit/media/blog/?id=38-7429041
- Atelier d'empathie pour l'A11y, sympa (en) : https://sites.google.com/state.co.us/tap-empathy-lab/home

## Autres

- Introduction sur le validisme et comment en sortir, super (en) : https://neurodivergentinsights.substack.com/p/what-ableism-looks-like-examples
- Nouveau formulaire MDPH en ligne, pas beau : https://lesdevalideuses.org/simplification-ou-effacement-le-nouveau-formulaire-mdph-une-violence-de-plus/
- Construire un vérificateur de contraste des images dans le navigateur, amusant (en) : https://www.timdixon.net/blog/2026/05/how-i-built-a-free-image-colour-contrast-checker-that-runs-entirely-in-your-browser/
