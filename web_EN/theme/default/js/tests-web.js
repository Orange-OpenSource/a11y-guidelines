	...
	let app = new function() {
	  ...
	  this.UpdateFeedback = function(activeFilter, nbTests) {
		...
		if (activeFilter) {
			...
			htmlFeedback = '<p><span><b>'+nbTests+'</b> tests in progress</span> | <button type="button" class="btn btn-secondary btn-sm" id="reinitLink">reset</a></p>';
			...
			htmlFeedback = '<p><b>'+nbTests+'</b> tests in progress</p>';
			elFeedback.innerHTML = htmlFeedback;
		}
	  ...
	  this.FetchAll = function(currentRefTests) {
		  ...
		  for (let i in currentRefTests) {
			...
			htmlrefTests += '</div></article>';
			  ...
			  currentRefTests.length===0 ?  elrefTests.innerHTML = '<div class="alert alert-warning">No result matches your selection</div>' : elrefTests.innerHTML = htmlrefTests;
		...
		this.DisplayFilters = function() {
			  ...
			  htmlFilterFooter += '<button id="reinit" class="btn btn-secondary" disabled>Reset</button>';
			 ...
			 elBtnReinit.addEventListener('click', function() {
			  ...
			  htmlTypes += '<li><input type="radio" id="typeAll" name="types" value="typeAll" checked> <label for="typeAll" >All</label>';
