...
index.add({
    ...
    title: "5. Use lists",    
    body: "## 5. Use lists  **Target: ** everyone, and especially people with visual impairments and cognitive limitations.   **When: ** as of design and during content writing.  **Description: **   Pages, if required, must contain semantically relevant lists.  Add semantics to the &lt;abbr&gt;HTML&lt;/abbr&gt; content using the following tags:  - `ol` for ordered lists  - `ul` for unordered lists  - `li` for list items  **Checklist: **  - There must be no empty list. - There must be no list with a single element (because this is not a list, unless this list is generated dynamically!).  **Users’ goal: **  Using lists allows users to identify, consolidate and sequentially display elements of the same kind.  **Technical goal: **   Requires coherent structuring of the content by the creator of the site.  **Reference: ** &lt;abbr&gt;WCAG&lt;/abbr&gt; 1.3.1  "
});
...
documentTitles["fondamentaux-nontech.html#9-link-labels"] = "9. Link labels";
index.add({
    url: "fondamentaux-nontech.html#9-link-labels",
    title: "9. Link labels",    
    body: "## 9. Link labels **Target: ** everyone and especially people with visual impairments, cognitive limitations or attention difficulties.   **When: ** when designing the service and during graphic design.  **Description:**   The wording of the links should be explicit enough. In exceptional cases when it is technically impossible, provide a proper label so a screen reader can use it.  **Do:**   `Learn about our offers`   **Don’t:**   `click here`   `more details`  "
});
