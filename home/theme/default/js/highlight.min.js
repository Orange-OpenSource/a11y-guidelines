...
//            if($(n).hasClass('lang-' + lg[0])) {
	...
	codeBlocks.each(function(idx, n){
		...
		if(!n.nextSibling || codeBlocks.index(n.nextSibling) < 0){
			$('<ul class="source-nav">'+nav.map(function(nn,j){
				return '<li data-code-block="'+i + '-' + j+'"'+(j==0?' class="active"':'')+'><button>'+nn+'</button></li>';
	...
	$('li[data-code-block] button').on('click', function(e){
		...
		$('li[data-code-block^='+codeBlockId.replace(/\d+$/, '')+']').removeClass('active');
