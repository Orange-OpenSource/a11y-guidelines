...
index.add({
    ...
    body: "## 5. Définir des équivalents textuels **Cible : ** les personnes déficientes visuelles, les personnes malentendantes ou déficientes cognitives et les moteurs de recherche.   **Quand : ** dès la conception et pendant le développement.  **Description :**   Mettre en place des alternatives textuelles à tous les éléments informatifs non-textuels (alternatives aux images, icones). De même prévoir des scripts ou des sous-titres pour les contenus audio ou vidéo. Prévoir des alternatives audio pour les captcha.  **Exemple : **   Dans la capture ci-dessous, il faudrait par exemple prévoir dès la conception les textes alternatifs pour chaque bouton : - menu - réglages - chaine précédente - couper le son ...    ![capture d'écran d'un lecteur vidéo contenant plusieurs boutons](images/player.png)       **Note à propos des CAPTCHA :**   Les captcha sont souvent la source de difficultés pour les utilisateurs. Si la mise en place d'un système anti-spam ne peut être évitée, il est souhaitable de s'orienter vers une solution plus souple pour l'utilisateur :  - Champ de formulaire caché à laisser vide (technique du Honeypot), non-visibles pour l'utilisateur.  - Test logique (question dont la réponse est évidente, test mathématique simple ...).  - Double authentification.  Si la présence d'un captcha ne peut être évitée, il est indispensable de **prévoir une alternative pour les captcha uniquement visuels ou sonores**.  "
});
...
index.add({
    ...
    body: "## Introduction : l'accessibilité des mails Les mails au format texte et sans image, comme le sont les mails entre salariés, sont généralement accessibles. Le problème d'inaccessibilité est plutôt le fait des pushmails. Ces mails, à la mise en forme évoluée (liens, mise en page complexe, illustrations, schémas), permettent la publication de quantité d'informations et sont un outil essentiel de la communication d'entreprise.  "
});
...
index.add({
    ...
    body: "### Simplifier la rédaction du contenu  **Pour qui : ** tout le monde et en particulier, les personnes déficientes visuelles, cognitives, avec des difficultés pour lire ou avec un déficit d'attention.    **Description : **   Simplifier le rédactionnel pour tous les utilisateurs.  **À vérifier : **   S'assurer que le rédactionnel : - Utilise des tournures de phrases limpides     - des phrases courtes et à la construction simple : sujet, verbe, complément     - une idée par phrase     - utiliser des mots simples et expliquer les mots compliqués     - utiliser le présent     - privilégier le style direct et la forme active (pas de subjonctif, pas de conjonctions) - Ne contient pas de mots en anglais, de sigles ou abréviations sauf ceux passés dans le langage courant  - Utilise les caractères typographiques du Français       - Tiret - , cadratin —, « guillemets français »     - Accents même aux majuscules (éèêçàïùÉÈÊÇÀÏÙ)  **Objectif : **   Permettre aux utilisateurs de comprendre facilement et d'interpréter le sens du contenu.  "
});
...
index.add({
    ...
    body: "### Simplifier la présentation  **Pour qui : ** tout le monde et en particulier, les personnes déficientes visuelles, cognitives, avec des difficultés pour lire ou avec un déficit d'attention.    **Description : **   Simplifier la présentation du contenu et respecter les bonnes pratiques en terme de présentation.   **À vérifier : **   Pour assurer une lecture facile : - Aligner le texte à gauche, mais ne pas le justifier - Utiliser une police de caractères facilement lisible en corps 12pt minimum - Mettre  en place une ponctuation simple - Ne pas mettre d’italiques, ni de phrases en majuscules, mais une majuscule en début de phrase - Éviter les fonds texturés (images) - Ne pas faire de retours chariots répétés ou de tabulation pour espacer (mais la fonction Paragraphe Espacement ou Retrait dans Word) - Éviter les tableaux de mise en page ou de données - Mettre en place un indicateur de fin de mail (par exemple, une image indiquant « fin de mail », un texte blanc sur fond blanc \&quot;fin de mail\&quot;)  **Objectif : ** S'assurer que la présentation n'empêche pas l'accès au contenu et à sa compréhension.  "
});
...
index.add({
    ...
    body: "### Donner une alternative aux images  **Pour qui : ** tout le monde et en particulier, les personnes déficientes visuelles ou cognitives.    **Description : **   Mettre en place des alternatives textuelles à tous les éléments informatifs non-textuels (alternatives aux images, icones...).  Mettre pour toute image (balise `img` en HTML) une alternative pertinente (un attribut `alt` en HTML) :  - Pour les images cliquables : préciser la fonction / la cible du lien dans l’alternative, - Pour les images porteuses d'information : mettre une alternative décrivant l'information contenue dans l'image - Pour les images contenant du texte : mettre une alternative reprenant au moins le texte de l’image, - Pour les images décoratives : mettre une alternative vide, - Pour les images dont le contenu de l'alternative serait trop long (schémas, graphes...), mettre à proximité de l'image à décrire, sa description sous forme de texte dans le mail ou, un lien pointant vers une page html contenant cette description.  **À vérifier : **   Toute image doit posséder une  alternative qui ne reflète que le contenu informatif qu'elle porte. Pour les graphes, la solution, peut être, de mettre un lien sous l'image permettant d'accéder à un tableau de chiffre en bas du mail, par exemple. Une alternative doit être : - Précise et équivalente au contenu de l'image ou sa fonction - Succincte, quelques mots à deux phrases courtes semble la bonne taille - Pas redondante, ne pas fournir la même information que dans le texte environnant - Pas de texte générique, pas besoin d'alternative du type \&quot;image de...\&quot;, \&quot;graphique sur...\&quot;, \&quot;lien vers...\&quot;, ...  **Objectif : **   Permettre l'accès à l'information incluse dans une image pour des utilisateurs qui n'y ont pas accès. Point bloquant : une image sans description textuelle est inexploitable par des personnes ayant des déficiences visuelles ou celles n'affichant pas les images (mobile, faible bande passante...).  **Exemple : **   word  "
});
...
index.add({
    ...
    body: "### Spécifier la langue du mail et les changements éventuels  **Pour qui : ** les personnes déficientes visuelles.    **Description : **   Spécifier la langue principale du document (l'attribut `lang` et/ou `xml:lang` dans la balise `html`).  Préciser également la langue d'un contenu exprimé dans une langue autre que la langue principale (l'attribut `lang` et/ou `xml:lang` dans l'élément html contenant l'expression en langue étrangère).    **À vérifier : **   Pour les mots ou les expressions en langue étrangère passés dans l'usage courant (Google, newsletter...) ou les noms propres, ne pas indiquer de changement de langue.  **Objectif : **   Cette indication permet d'adapter la synthèse vocale pour une bonne prononciation de la langue à utiliser.  **Exemple pour une page en français : **   Word !!!!!!!!!!!!!!!! - pour du HTML : `&lt;html lang=\&quot;fr\&quot;&gt;` - pour du XHTML : `&lt;html xmlns=\&quot;http://www.w3.org/1999/xhtml\&quot; xml:lang=\&quot;fr\&quot; lang=\&quot;fr\&quot;&gt;`  **Exemple de changement de langue : **   `découvrir Orange &lt;span lang=\&quot;fr\&quot;&gt;live&lt;/span&gt; TV`  "
});

documentTitles["pushmail.html#fournir-une-mise-en-page-accessible"] = "Fournir une mise en page accessible";
index.add({
    url: "pushmail.html#fournir-une-mise-en-page-accessible",
    title: "Fournir une mise en page accessible",    
    body: "### Fournir une mise en page accessible  **Pour qui : ** les personnes déficientes visuelles.    **Description : **   Simplifier la mise en page bien que les clients de messagerie (Outlook inclus) obligent à utiliser une mise en page en tableau.  **À vérifier : **   Globalement, ne pas utiliser de mise en page complexe (fusion de cellules d'un tableau, multicolonnage...),  - Éviter d'imbriquer les tableaux de mise en page (grand maximum, 2 niveaux d'imbrication), mais aussi :     - Ne pas fusionner les cellules d’un tableau, mais utiliser plusieurs tableaux plus simples     - Pas de cellules vides     - Pas d’images de fond  - Assurer une lecture séquentielle (on doit pouvoir lire de gauche à droite et de haut en bas) - Éviter les tableaux de données, les graphiques ou les schémas complexes  **Objectif : **   Permettre, notamment aux utilisateurs de lecteurs d'écran, de comprendre facilement le contenu quelle que soit la mise en page utilisée.  "
});
...
index.add({
    ...
    body: "### Identifier les liens et leurs cibles  **Pour qui : ** tout le monde, en particulier, pour les malvoyants, les non-voyants et les personnes ayant des difficultés de lecture.  **Description : **   Rendre les liens identifiables et compréhensibles hors contexte  **À vérifier : **   - Souligner les liens, car sinon les personnes doivent faire un effort cognitif pour les trouver. Les personnes en situation de contrainte motrice ne peuvent les trouver qu’en naviguant davantage. - Ne pas utiliser des intitulés comme \&quot;Cliquez ici\&quot;, \&quot;Pour en savoir plus\&quot;, \&quot;Suivez ce lien\&quot;, \&quot;Plus de détails\&quot;, \&quot;Lire la suite\&quot;, mais des intitulés qui identifient  bien leur cible - Indiquer l’ouverture de toute nouvelle fenêtre (adresse e-mail, site internet…) en ajoutant à l'intitulé du texte, par exemple \&quot;nouvelle fenêtre\&quot;, car le multifenêtrage qui peut être déjà troublant pour un voyant est tout simplement bloquant pour un non-voyant.  **Objectif : **   Permettre à un utilisateur n'ayant pas accès au contexte visuel de connaître la destination du lien. Notamment important pour les utilisateurs naviguant grâce à une liste de liens extraite de la page (lecteurs d'écran) ou les utilisateurs de loupe logicielle qui ne voient qu'une fraction de la page.   "
});
